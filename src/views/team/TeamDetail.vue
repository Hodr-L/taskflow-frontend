<template>
  <div class="team-detail-container">
    <!-- 杩斿洖鎸夐挳 -->
    <div class="back-button">
      <el-button type="text" @click="goBack">
        <el-icon><ArrowLeft /></el-icon>
        杩斿洖鍥㈤槦鍒楄〃
      </el-button>
    </div>

    <!-- 鍥㈤槦澶撮儴淇℃伅 -->
    <div class="team-header">
      <div class="team-basic-info">
        <div class="team-avatar-section">
          <el-avatar :size="80" :src="team.avatar_url">
            {{ team.name?.substring(0, 2).toUpperCase() }}
          </el-avatar>
          <div class="team-avatar-actions">
            <el-button type="text" size="small" @click="showAvatarUpload = true">
              鏇存崲澶村儚
            </el-button>
          </div>
        </div>

        <div class="team-info">
          <h1 class="team-name">{{ team.name }}</h1>
          <p class="team-description">{{ team.description }}</p>

          <div class="team-meta">
            <div class="meta-item">
              <el-icon><User /></el-icon>
              <span>{{ team.member_count || 0 }} 鍚嶆垚鍛?/span>
            </div>
            <div class="meta-item">
              <el-icon><Folder /></el-icon>
              <span>{{ team.project_count || 0 }} 涓」鐩?/span>
            </div>
            <div class="meta-item">
              <el-icon><Calendar /></el-icon>
              <span>鍒涘缓浜?{{ formatDate(team.created_at) }}</span>
            </div>
          </div>
        </div>

        <div class="team-actions">
          <el-dropdown @command="handleTeamAction">
            <el-button type="primary">
              鍥㈤槦鎿嶄綔
              <el-icon class="el-icon--right"><ArrowDown /></el-icon>
            </el-button>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="edit">缂栬緫鍥㈤槦淇℃伅</el-dropdown-item>
                <el-dropdown-item command="invite">閭€璇锋垚鍛?/el-dropdown-item>
                <el-dropdown-item command="settings">鍥㈤槦璁剧疆</el-dropdown-item>
                <el-dropdown-item divided command="leave" v-if="userRole !== 'owner'"
                  >閫€鍑哄洟闃?/el-dropdown-item
                >
                <el-dropdown-item command="delete" v-if="userRole === 'owner'"
                  >瑙ｆ暎鍥㈤槦</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </div>
      </div>
    </div>

    <!-- 鏍囩椤?-->
    <el-tabs v-model="activeTab" class="team-tabs">
      <el-tab-pane label="鎴愬憳绠＄悊" name="members">
        <TeamMembersTab
          :team-id="teamId"
          :members="members"
          :user-role="userRole"
          @refresh="loadTeamData"
        />
      </el-tab-pane>

      <el-tab-pane label="椤圭洰绠＄悊" name="projects">
        <TeamProjectsTab :team-id="teamId" :projects="projects" :user-role="userRole" />
      </el-tab-pane>

      <el-tab-pane label="鍥㈤槦璁剧疆" name="settings">
        <TeamSettingsTab :team="team" :user-role="userRole" @update-team="handleTeamUpdate" />
      </el-tab-pane>

      <el-tab-pane label="娲诲姩鏃ュ織" name="activity">
        <TeamActivityTab :team-id="teamId" />
      </el-tab-pane>
    </el-tabs>

    <!-- 缂栬緫鍥㈤槦瀵硅瘽妗?-->
    <el-dialog v-model="showEditDialog" title="缂栬緫鍥㈤槦淇℃伅" width="500px">
      <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-width="80px">
        <el-form-item label="鍥㈤槦鍚嶇О" prop="name">
          <el-input
            v-model="editForm.name"
            placeholder="璇疯緭鍏ュ洟闃熷悕绉?
            maxlength="50"
            show-word-limit
          />
        </el-form-item>

        <el-form-item label="鍥㈤槦鎻忚堪" prop="description">
          <el-input
            v-model="editForm.description"
            type="textarea"
            :rows="3"
            placeholder="璇疯緭鍏ュ洟闃熸弿杩?
            maxlength="200"
            show-word-limit
          />
        </el-form-item>

        <el-form-item label="闅愮璁剧疆" prop="privacy">
          <el-radio-group v-model="editForm.privacy">
            <el-radio label="public">鍏紑锛堟墍鏈変汉鍙锛?/el-radio>
            <el-radio label="private">绉佹湁锛堜粎鎴愬憳鍙锛?/el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>

      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showEditDialog = false">鍙栨秷</el-button>
          <el-button type="primary" :loading="updating" @click="handleUpdateTeam">
            淇濆瓨淇敼
          </el-button>
        </span>
      </template>
    </el-dialog>

    <!-- 閭€璇锋垚鍛樺璇濇 -->
    <el-dialog v-model="showInviteDialog" title="閭€璇锋垚鍛? width="500px">
      <el-form ref="inviteFormRef" :model="inviteForm" :rules="inviteRules" label-width="80px">
        <el-form-item label="閭鍦板潃" prop="email">
          <el-input v-model="inviteForm.email" placeholder="璇疯緭鍏ヨ閭€璇风殑鎴愬憳閭" />
        </el-form-item>

        <el-form-item label="鎴愬憳瑙掕壊" prop="role">
          <el-select v-model="inviteForm.role" placeholder="璇烽€夋嫨鎴愬憳瑙掕壊">
            <el-option label="鎴愬憳" value="member" />
            <el-option label="绠＄悊鍛? value="admin" />
          </el-select>
        </el-form-item>

        <el-form-item label="閭€璇锋秷鎭? prop="message">
          <el-input
            v-model="inviteForm.message"
            type="textarea"
            :rows="3"
            placeholder="鍙€夌殑閭€璇锋秷鎭?
            maxlength="200"
            show-word-limit
          />
        </el-form-item>
      </el-form>

      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showInviteDialog = false">鍙栨秷</el-button>
          <el-button type="primary" :loading="inviting" @click="handleInviteMember">
            鍙戦€侀個璇?          </el-button>
        </span>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, reactive, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import { ArrowLeft, ArrowDown, User, Folder, Calendar } from '@element-plus/icons-vue'

// 瀵煎叆瀛愮粍浠?import TeamMembersTab from './components/TeamMembersTab.vue'
import TeamProjectsTab from './components/TeamProjectsTab.vue'
import TeamSettingsTab from './components/TeamSettingsTab.vue'
import TeamActivityTab from './components/TeamActivityTab.vue'
import type { Team } from '../../types/team'
import type { Member } from '../../types/member'

const route = useRoute()
const router = useRouter()
const teamId = ref(Number(route.params.id))

// 鍥㈤槦鏁版嵁
const team = ref<Team>({
  id: teamId.value,
  name: '鍓嶇寮€鍙戝洟闃?,
  description: '璐熻矗鍓嶇鐣岄潰寮€鍙戝拰鐢ㄦ埛浣撻獙浼樺寲',
  avatar_url: '',
  member_count: 8,
  project_count: 3,
  owner_id: 1,
  privacy: 'private',
  created_at: '2026-01-15T10:30:00Z',
})

// 鐢ㄦ埛瑙掕壊锛堝綋鍓嶇敤鎴峰湪鍥㈤槦涓殑瑙掕壊锛?const userRole = ref<'owner' | 'admin' | 'member'>('owner') // owner|admin|member

// 鎴愬憳鏁版嵁
const members = ref<Member[]>([
  {
    id: 1,
    username: 'zhangsan',
    email: 'zhangsan@example.com',
    fullname: '寮犱笁',
    bio: '鍥㈤槦璐熻矗浜?,
    avatar_url: '',
    role: 'owner',
    joined_at: '2026-01-15T10:30:00Z',
  } as Member,
  {
    id: 2,
    username: 'lisi',
    email: 'lisi@example.com',
    fullname: '鏉庡洓',
    bio: '绠＄悊鍛?,
    avatar_url: '',
    role: 'admin',
    joined_at: '2026-01-20T14:15:00Z',
  } as Member,
  {
    id: 3,
    username: 'wangwu',
    email: 'wangwu@example.com',
    fullname: '鐜嬩簲',
    bio: '寮€鍙戝伐绋嬪笀',
    avatar_url: '',
    role: 'member',
    joined_at: '2026-02-01T09:00:00Z',
  } as Member,
])

// 椤圭洰鏁版嵁
const projects = ref([
  {
    id: 1,
    name: 'TaskFlow 鍓嶇寮€鍙?,
    description: '浠诲姟绠＄悊绯荤粺鍓嶇鐣岄潰寮€鍙?,
    status: 'active',
    task_count: 24,
    completed_tasks: 12,
    progress: 50,
    start_date: '2026-01-20',
    end_date: '2026-03-20',
  },
  {
    id: 2,
    name: '鐢ㄦ埛涓績閲嶆瀯',
    description: '鐢ㄦ埛涓績妯″潡閲嶆瀯鍜屼紭鍖?,
    status: 'active',
    task_count: 18,
    completed_tasks: 9,
    progress: 50,
    start_date: '2026-02-01',
    end_date: '2026-03-15',
  },
])

// 鐘舵€?const activeTab = ref('members')
const showEditDialog = ref(false)
const showInviteDialog = ref(false)
const showAvatarUpload = ref(false)
const updating = ref(false)
const inviting = ref(false)

// 缂栬緫琛ㄥ崟
const editForm = reactive({
  name: team.value.name,
  description: team.value.description,
  privacy: team.value.privacy,
})

const editRules = {
  name: [
    { required: true, message: '璇疯緭鍏ュ洟闃熷悕绉?, trigger: 'blur' },
    { min: 2, max: 50, message: '鍥㈤槦鍚嶇О闀垮害鍦?鍒?0涓瓧绗︿箣闂?, trigger: 'blur' },
  ],
  description: [
    { required: true, message: '璇疯緭鍏ュ洟闃熸弿杩?, trigger: 'blur' },
    { max: 200, message: '鍥㈤槦鎻忚堪涓嶈兘瓒呰繃200涓瓧绗?, trigger: 'blur' },
  ],
}

// 閭€璇疯〃鍗?const inviteForm = reactive({
  email: '',
  role: 'member',
  message: '',
})

const inviteRules = {
  email: [
    { required: true, message: '璇疯緭鍏ラ偖绠卞湴鍧€', trigger: 'blur' },
    { type: 'email', message: '璇疯緭鍏ユ纭殑閭鍦板潃', trigger: 'blur' },
  ],
  role: [{ required: true, message: '璇烽€夋嫨鎴愬憳瑙掕壊', trigger: 'change' }],
}

// 鐢熷懡鍛ㄦ湡
onMounted(() => {
  loadTeamData()
})

// 鍔犺浇鍥㈤槦鏁版嵁
const loadTeamData = async () => {
  try {
    // TODO: 璋冪敤API鑾峰彇鍥㈤槦鏁版嵁
    // const response = await teamApi.getTeamDetail(teamId.value)
    // team.value = response.data.team
    // members.value = response.data.members
    // projects.value = response.data.projects
    // userRole.value = response.data.user_role

    console.log('鍔犺浇鍥㈤槦鏁版嵁:', teamId.value)
  } catch (error) {
    ElMessage.error('鍔犺浇鍥㈤槦鏁版嵁澶辫触')
  }
}

// 杩斿洖鍥㈤槦鍒楄〃
const goBack = () => {
  router.push('/teams')
}

// 澶勭悊鍥㈤槦鎿嶄綔
const handleTeamAction = (command: string) => {
  switch (command) {
    case 'edit':
      showEditDialog.value = true
      break
    case 'invite':
      showInviteDialog.value = true
      break
    case 'settings':
      activeTab.value = 'settings'
      break
    case 'leave':
      handleLeaveTeam()
      break
    case 'delete':
      handleDeleteTeam()
      break
  }
}

// 鏇存柊鍥㈤槦淇℃伅
const handleUpdateTeam = async () => {
  updating.value = true

  try {
    // TODO: 璋冪敤鏇存柊鍥㈤槦鐨凙PI
    await new Promise((resolve) => setTimeout(resolve, 1000))

    team.value.name = editForm.name
    team.value.description = editForm.description
    team.value.privacy = editForm.privacy

    ElMessage.success('鍥㈤槦淇℃伅鏇存柊鎴愬姛')
    showEditDialog.value = false
  } catch (error) {
    ElMessage.error('鏇存柊澶辫触锛岃閲嶈瘯')
  } finally {
    updating.value = false
  }
}

// 閭€璇锋垚鍛?const handleInviteMember = async () => {
  inviting.value = true

  try {
    // TODO: 璋冪敤閭€璇锋垚鍛樼殑API
    await new Promise((resolve) => setTimeout(resolve, 1000))

    ElMessage.success('閭€璇峰凡鍙戦€?)
    showInviteDialog.value = false
    inviteForm.email = ''
    inviteForm.message = ''
  } catch (error) {
    ElMessage.error('閭€璇峰け璐ワ紝璇烽噸璇?)
  } finally {
    inviting.value = false
  }
}

// 閫€鍑哄洟闃?const handleLeaveTeam = async () => {
  try {
    await ElMessageBox.confirm('纭畾瑕侀€€鍑鸿繖涓洟闃熷悧锛?, '纭閫€鍑?, {
      confirmButtonText: '纭畾閫€鍑?,
      cancelButtonText: '鍙栨秷',
      type: 'warning',
    })

    // TODO: 璋冪敤閫€鍑哄洟闃熺殑API
    await new Promise((resolve) => setTimeout(resolve, 1000))

    ElMessage.success('宸查€€鍑哄洟闃?)
    router.push('/teams')
  } catch {
    // 鐢ㄦ埛鍙栨秷
  }
}

// 瑙ｆ暎鍥㈤槦
const handleDeleteTeam = async () => {
  try {
    await ElMessageBox.confirm(
      '纭畾瑕佽В鏁ｈ繖涓洟闃熷悧锛熸鎿嶄綔灏嗗垹闄ゅ洟闃熸墍鏈夋暟鎹笖涓嶅彲鎭㈠銆?,
      '纭瑙ｆ暎',
      {
        confirmButtonText: '纭畾瑙ｆ暎',
        cancelButtonText: '鍙栨秷',
        type: 'error',
      },
    )

    // TODO: 璋冪敤瑙ｆ暎鍥㈤槦鐨凙PI
    await new Promise((resolve) => setTimeout(resolve, 1000))

    ElMessage.success('鍥㈤槦宸茶В鏁?)
    router.push('/teams')
  } catch {
    // 鐢ㄦ埛鍙栨秷
  }
}

// 澶勭悊鍥㈤槦鏇存柊
const handleTeamUpdate = (updatedTeam: any) => {
  team.value = { ...team.value, ...updatedTeam }
}

// 鏍煎紡鍖栨棩鏈?const formatDate = (dateString?: string) => {
  if (!dateString) return ''
  const date = new Date(dateString)
  return date.toLocaleDateString('zh-CN')
}
</script>

<style scoped>
.team-detail-container {
  padding: 20px;
  max-width: 1200px;
  margin: 0 auto;
}

.back-button {
  margin-bottom: 20px;
}

.team-header {
  background: white;
  border-radius: 12px;
  padding: 30px;
  margin-bottom: 24px;
  box-shadow: 0 2px 12px rgba(0, 0, 0, 0.08);
}

.team-basic-info {
  display: flex;
  align-items: flex-start;
  gap: 30px;
}

.team-avatar-section {
  flex-shrink: 0;
  text-align: center;
}

.team-avatar-actions {
  margin-top: 12px;
}

.team-info {
  flex: 1;
}

.team-name {
  font-size: 28px;
  font-weight: 600;
  color: #303133;
  margin-bottom: 12px;
}

.team-description {
  font-size: 16px;
  color: #606266;
  line-height: 1.6;
  margin-bottom: 20px;
}

.team-meta {
  display: flex;
  gap: 24px;
  flex-wrap: wrap;
}

.meta-item {
  display: flex;
  align-items: center;
  gap: 8px;
  color: #909399;
  font-size: 14px;
}

.meta-item .el-icon {
  font-size: 16px;
}

.team-actions {
  flex-shrink: 0;
}

.team-tabs {
  background: white;
  border-radius: 12px;
  padding: 20px;
  box-shadow: 0 2px 12px rgba(0, 0, 0, 0.08);
}

:deep(.el-tabs__header) {
  margin-bottom: 24px;
}

:deep(.el-tabs__item) {
  font-size: 16px;
  font-weight: 500;
}

/* 鍝嶅簲寮忚璁?*/
@media (max-width: 768px) {
  .team-detail-container {
    padding: 16px;
  }

  .team-basic-info {
    flex-direction: column;
    align-items: center;
    text-align: center;
    gap: 20px;
  }

  .team-meta {
    justify-content: center;
  }

  .team-actions {
    width: 100%;
  }

  .team-actions .el-dropdown {
    width: 100%;
  }

  .team-actions .el-button {
    width: 100%;
  }
}

@media (max-width: 480px) {
  .team-header {
    padding: 20px;
  }

  .team-name {
    font-size: 24px;
  }

  .team-description {
    font-size: 14px;
  }
}
</style>
